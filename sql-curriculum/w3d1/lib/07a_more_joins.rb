...
def heart_tracks
  ...
  JOIN
    tracks ON albums.asin = tracks.album
  WHERE
    tracks.song LIKE '%Heart%' AND albums.asin IN (
    SELECT
...
def expensive_tastes
  ...
  # along with the average price per track. One or more of each aggregate functions,
  ...
  execute(<<-SQL)
    SELECT
      styles.style, AVG(price_average)
    FROM
      styles
    JOIN
      (
      SELECT
        albums.asin, albums.price / COUNT(tracks.*) AS price_average
      FROM
        tracks
      JOIN
        albums ON tracks.album = albums.asin
      ...
      GROUP BY
        albums.asin
      ) AS price_per_track ON styles.album = price_per_track.asin
